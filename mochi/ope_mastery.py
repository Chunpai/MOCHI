...
data = pickle.load(open("../data/MasteryGrids/train_bandit_feedback.pkl", "rb"))
n_rounds = data["n_rounds"]
n_actions = data['n_actions']
indices = list(range(n_rounds))
splits = train_test_split(indices, test_size=0.2, shuffle=True)
train_test_indices, val_indices = splits
train_indices = train_test_indices[:5000]
test_indices = train_test_indices[5000:]

# pscore is the propensity score of behavior policy (log-policy)
train_context = data["context"][train_indices]
train_action = data["action"][train_indices]
train_reward = data["reward"][train_indices]
train_pscore = data["pscore"][train_indices]  ### Check this. Try popularity as Log-policy
train_expected_reward = data["expected_reward"][train_indices]
train_action_context = data["action_context"].copy()

test_context = data["context"][test_indices]
test_action = data["action"][test_indices]
test_reward = data["reward"][test_indices]
test_pscore = data["pscore"][test_indices]
test_expected_reward = data["expected_reward"][test_indices]
test_action_context = data["action_context"].copy()
...
for num_data in num_data_list:
    ...
    for _ in tqdm(range(num_runs), desc=f"num_data={num_data}..."):
        ...
        sample_val_indices = val_indices[:num_data]
        val_context = data["context"][sample_val_indices]
        val_action = data["action"][sample_val_indices]
        val_reward = data["reward"][sample_val_indices]
        val_pscore = data["pscore"][sample_val_indices]
        val_expected_reward = data["expected_reward"][sample_val_indices]
        val_action_context = data["action_context"].copy()
