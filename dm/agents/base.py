...
class BaseAgent:
    ...
    def load_checkpoint(self, file_name):
        ...
        try:
            ...
            self.best_auc_threshold = checkpoint['best_auc_threshold']
    ...
    def save_checkpoint(self, file_name="checkpoint.pth.tar", is_best=0):
        ...
        state = {
            ...
            'best_auc_threshold': self.best_auc_threshold
        }
        # Save the state
    ...
    def track_best(self, true_labels, pred_labels):
        ...
        if self.metric == "rmse":
            ...
            fpr, tpr, thresholds = metrics.roc_curve(self.true_labels, self.pred_labels)
            J = tpr - fpr
            idx = np.argmax(J)
            ...
            if perf > self.best_val_perf:
                self.best_val_perf = perf
                self.best_auc_threshold = best_threshold
                self.best_train_loss = self.train_loss.item()
                self.best_epoch = self.current_epoch
        else:
            raise AttributeError
